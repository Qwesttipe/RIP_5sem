	...
	for _, mmo := range mmos {
		if mmo.CPU.Valid && order.IOPS.Valid && order.DBcache.Valid {
			mmo.RAM = sql.NullFloat64{
				Float64: order.RAM_OS.Float64 + (order.IOPS.Float64 * order.IOPS.Float64) + order.DBcache.Float64*1.25,
			}

			// нагрузочный расход
			mmo.CPU = sql.NullFloat64{
				Float64: order.RPS.Float64 / order.IOPS.Float64 * order.ARPT.Float64 / (1 - order.CPUload.Float64),
				Valid:   true,
			}
